<ul component="category" class="topics-list list-unstyled" itemscope itemtype="http://www.schema.org/ItemList" data-nextstart="{nextStart}" data-set="{set}">
	...
	<li component="category/topic" class="category-item border-bottom py-3 py-lg-4 d-flex flex-column flex-lg-row align-items-start {function.generateTopicClass}" <!-- IMPORT partials/data/category.tpl -->>
		...
		<div class="d-flex p-0 col-12 col-lg-7 gap-2 gap-lg-3 pe-1 align-items-start">
			...
			{{{ if ./thumbs }}}
					<div component="topic/thumb" class="topic-thumb"> 
						<a class="text-reset" href="{config.relative_path}/topic/{./slug}{{{ if ./bookmark }}}/{./bookmark}{{{ end }}}" >
							<img 
								data-sizes="auto"
								data-srcset="{./thumbs.0.url}"
								style="border-radius: 5px;"
								src="{./thumbs.0.url}"
								class="shadow-sm bg-light" />
							<div id="tooltip" role="tooltip"> <img width="350" height="auto" src="{./thumbs.0.url}" class="not-responsive" /></div>
						</a>
					</div>
				{{{ else }}}
				...
				{{{ if showSelect }}}
				<div class="checkbox position-absolute top-100 start-50 translate-middle-x pt-1 m-0 d-none d-lg-flex" style="max-width:max-content">
					<i component="topic/select" class="fa text-muted pointer fa-square-o p-1"></i>
				</div>
				{{{ end }}}
			</div>
			<div class="flex-grow-1 d-flex flex-column align-items-start gap-1 position-relative ">
				...
				<span component="topic/labels" class="d-flex flex-wrap gap-1">
					...
					<span data-tid="{./tid}" component="topic/tags" class="lh-1 tag-list hidden-xs d-flex flex-wrap gap-1 {{{ if !./tags.length }}}hidden{{{ end }}}">
						...
						<span class="text-body d-block mx-2">
							<i class="fa fa-heart-o"></i><span class="text-muted px-1">{./votes}</span>	
							<i class="fa fa-eye"></i><span class="text-muted px-1">{./viewcount}</span>
						</span>
				</span>

				<!-- 移动端显示时间 -->
				<span class="d-sm-block d-md-none badge bg-transparent text-muted fw-normal timeago" title="{{{ if ./teaser.timestampISO }}}{./teaser.timestampISO}{{{ else }}}{./timestampISO}{{{ end }}}"></span>
				{{{ if showSelect }}}
